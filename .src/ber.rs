...
//! frames with: a one-byte tag, a length in short or long form, the
//! contents. Nothing here knows what a tag means; MMS, GOOSE and SNMP say
//! that in their own crates, and each carried this file until 2026-09-14
//! (ADR-0044). The indefinite length form is refused, as both refuse it.

use crate::error::{Result, protocol_error};

/// The universal tags the technologies write.
...
/// An INTEGER's contents: two's complement, the fewest bytes that keep
/// the sign.
#[must_use]
...
///
/// # Errors
/// Cut short, a tag of the multi-byte form, a length of the indefinite
/// form or wider than four bytes, or a length past the end.
